...
Imports System.Net
Imports System.Xml
Partial Class Ebiz_sales_integration
        ...
        If Not IsPostBack Then
            ...
            If Not Request.QueryString("ORDER_NO") Is Nothing Then
                ...
                If (dsTransacId.Tables(0).Rows.Count > 0) Then
                    ...
                    strCmd1 = String.Format("alert('This order is already Pre-Authorized');window.opener.document.getElementById('" & Request.QueryString.Get("Retrive_id") & "').click();window.close();", strFile1)
                ...
                Else
                    ...
                    Insert_query("INSERT INTO SD_ORDER_CC_PAYMENT (ORDER_NO,LINE_NO,AMOUNT,PAYMENT_TYPE,API_TOKEN,CREATED_BY,CREATED_DATE,DOC_TYPE) select ORDER_NO,1,nvl(sum(line_total_amount),0) + (select to_number(nvl(value,0)) from sys_tool_settings where key='DEFAULT_SHIPPING_CHARGE') + (nvl(sum(line_total_amount),0) * (select to_number(nvl(value,0)) from sys_tool_settings where key='PERCENT_OF_EXTRA_CHARGES')/100)  Amount,'" & ptype & "','','" & Session("USER_ID") & "',SYSDATE,'AUTHONLY' from sd_order_detail where  ORDER_NO='" & Order_no & "'  group by order_no")
    ...
    Public Function SOAPManual(ByVal ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal url As String, ByVal action As String, ByVal TLSecurityID As String, ByVal TLClientURL As String, ByVal FromEmail As String, ByVal FromName As String) As String
        ...
        Ebiz_sales_integration.InsertSoapEnvelopeIntoWebRequest(soapEnvelopeXml, webRequest)
    ...
    Private Shared Function CreateSoapEnvelope(ByRef ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal TL_USER_ID As String, ByVal TL_PASSWORD As String, ByVal TL_SECURITY As String, ByVal TL_CLIENT As String, ByVal TL_FROM_EMAIL As String, ByVal FromName As String) As XmlDocument
        ...
        "            </ebiz:BillingAddress>" &
        "            <ebiz:ApprovedURL>" & TL_CLIENT & "eBiz_cc_sales_responce.aspx?PTYPE=" & ptype & "</ebiz:ApprovedURL>" &
        "            <ebiz:DeclinedURL>" & TL_CLIENT & "eBiz_cc_sales_responce.aspx?PTYPE=" & ptype & "</ebiz:DeclinedURL>" &
        "            <ebiz:ErrorURL>" & TL_CLIENT & "eBiz_cc_sales_responce.aspx?PTYPE=" & ptype & "</ebiz:ErrorURL>" &
        ...
        "            <ebiz:ProcessingCommand>AUTHONLY</ebiz:ProcessingCommand>" &
