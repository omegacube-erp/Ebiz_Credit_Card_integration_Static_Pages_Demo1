...
Partial Class eBiz_cc_sales_process_pay_responce
        ...
        Try
            ...
            If (Request.QueryString.Get("TranResult") = "Approved") Then
                ...
                If (ds12.Tables(0).Rows.Count > 0) Then
                    ...
                    Call Update_query("Update SD_ORDER_CC_PAYMENT Set AUTHCODE='" & Request.QueryString.Get("AuthCode") & "', PAYBYTYPE='" & Request.QueryString.Get("PayByType") & "',CCTYPE='" & Request.QueryString.Get("CCType") & "', MASKEDCC='" & Request.QueryString.Get("MaskedCC") & "',TRANREFNUM='" & Request.QueryString.Get("TranRefNum") & "',PMTOKEN='" & Request.QueryString.Get("PmToken") & "',CUSTTOKEN='" & Request.QueryString.Get("CustToken") & "',TRANRESULTCODE='" & Request.QueryString.Get("TranResultCode") & "',PAYMENT_RESPONSE='" & Request.QueryString.Get("TranResult") & "',AMOUNT='" & ds12.Tables(0).Rows(0)("AMOUNT") & "',CHANGED_DATE=SYSDATE,AUTH_TRANS_USER= UPPER('" & Session("user_id").ToString & "'),AUTH_TRANS_DATE=SYSDATE where ORDER_NO='" & ORDER_NO & "'")

                    Call execute_storeProcedure("SP_CREATE_CR_FROM_SD_ORDER", ds12.Tables(0).Rows(0)("ORDER_NO").ToString & "#" & ds12.Tables(0).Rows(0)("AMOUNT"), "P_ORDER_NO#P_BAL_AMT", "C#N")
                    'Response.Write("Update SD_INVOICE_CC_PAYMENT Set AUTHCODE='" & Request.QueryString.Get("AuthCode") & "', PayByType='" & Request.QueryString.Get("PayByType") & "',CCType='" & Request.QueryString.Get("CCType") & "', MaskedCC='" & Request.QueryString.Get("MaskedCC") & "',TranRefNum='" & Request.QueryString.Get("TranRefNum") & "',PmToken='" & Request.QueryString.Get("PmToken") & "',CustToken='" & Request.QueryString.Get("CustToken") & "',TranResultCode='" & Request.QueryString.Get("TranResultCode") & "',PAYMENT_RESPONSE='" & Request.QueryString.Get("TranResult") & "',INVOICE_NO='" & ds12.Tables(0).Rows(0)("INVOICE_NO") & "',CHANGED_DATE=SYSDATE,AUTH_TRANS_USER= UPPER('" & Session("user_id").ToString & "'),AUTH_TRANS_DATE=SYSDATE Where INVOICE_NO='" & ds12.Tables(0).Rows(0)("INVOICE_NO") & "'")
                    'Exit Sub

                    'Call execute_storeProcedure("SP_CREATE_CR_FROM_SD_INV", ds12.Tables(0).Rows(0)("ORDER_NO").ToString & "#" & calamount, "P_INVOICE_NO#P_BAL_AMT", "C#N")


                Else
                    Update_query("Update SD_ORDER_CC_PAYMENT Set PAYMENT_RESPONSE='" & Request.QueryString.Get("TranResult") & "',CHANGED_DATE=SYSDATE,AUTH_TRANS_USER= UPPER('" & Session("user_id").ToString & "'),AUTH_TRANS_DATE=SYSDATE Where ORDER_NO='" & ds12.Tables(0).Rows(0)("ORDER_NO") & "'")
                End If
            Else
