...
Imports System.Net
Imports System.Xml
Partial Class Ebiz_sales_payment_process_integration
    Inherits System.Web.UI.Page
    Protected dbad As New OleDbDataAdapter
    Public conn As New Dbconn
    Public Order_no, LINE_NO, resp, ptype As String
    Public TLurl, TLactionURL, TLPassword, TLUserId, SECURITY_ID, CLIENT_URL, FROM_NAME, FROM_EMAIL, EXTRA_CHARGE, DEFAULT_SHIPPING_CHARGE As String
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Not Request.QueryString("ORDER_NO") Is Nothing Then
                ...
                'dsTransacId = Return_record_set("SELECT INVOICE_NO,LINE_NO,round(AMOUNT,2) AMOUNT,PAYMENT_RESPONSE,AUTHCODE,AUTH_TRANS_USER,AUTH_TRANS_ID,AUTH_TRANS_DATE,API_TOKEN,API_TOKEN_SESSION,CHANGED_BY,CHANGED_DATE,CREATED_BY,CREATED_DATE FROM SD_ORDER_CC_PAYMENT Where AUTH_TRANS_DATE > sysdate-1/10 AND PAYMENT_RESPONSE='IN_PROCESS' AND UPPER(AUTH_TRANS_USER)=UPPER('" & Session("user_id").ToString & "')")
				
				dsTransacId = Return_record_set("select * from SD_ORDER_CC_PAYMENT where ORDER_NO='" & Order_no & "' AND AUTHCODE IS NOT NULL")
                dsTransacId.Clear()
                ...
                If (dsTransacId.Tables(0).Rows.Count > 0) Then
                    ...
                    strCmd1 = String.Format("alert('This order is already Processed');window.opener.document.getElementById('" & Request.QueryString.Get("Retrive_id") & "').click();window.close();", strFile1)
                    ' strCmd1 = String.Format("window.close();", strFile1)
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "GEN1", strCmd1, True)
                    Exit Sub
                Else
                    Delete_query("DELETE FROM SD_ORDER_CC_PAYMENT where ORDER_NO='" & Order_no & "'")
                    ' Insert_query("INSERT INTO SD_ORDER_CC_PAYMENT (ORDER_NO,LINE_NO,INVOICE_NO,AMOUNT,PAYMENT_TYPE,API_TOKEN,CREATED_BY,CREATED_DATE) select ORDER_NO,1,INVOICE_NO,balance_amount,'" & ptype & "','','" & Session("USER_ID") & "',SYSDATE FROM V_BALANCE_INVOICE_TO_RECEIVE WHERE NVL(balance_amount,0)>0 AND ORDER_NO='" & Order_no & "'")
                    'Amount getting from SD_ORDER_HEADER to insert into SD_ORDER_CC_PAYMENT
                    Dim dsamt As New Data.DataSet
                    Dim amt As String
                    dsamt = Return_record_set("Select ROUND(NVL(ADVANCE_AMOUNT,0),2) AS AMOUNT from SD_ORDER_HEADER WHERE ORDER_NO='" & Order_no & "' and ADVANCE_AMOUNT>0")
                    dsamt.Clear()
                    dbad.Fill(dsamt)
                    If (dsamt.Tables(0).Rows.Count > 0) Then
                        If Not (Equals(dsamt.Tables(0).Rows(0)("AMOUNT"), System.DBNull.Value)) Then
                            amt = dsamt.Tables(0).Rows(0)("AMOUNT").ToString
                        ...
                        End If
                        Insert_query("INSERT INTO SD_ORDER_CC_PAYMENT(ORDER_NO, LINE_NO, PAYMENT_TYPE,AMOUNT, CREATED_BY, CREATED_DATE,DOC_TYPE)values('" & Order_no & "','" & LINE_NO & "','" & ptype & "','" & amt & "','" & Session("USER_ID") & "',SYSDATE,'ADVANCE')")
                    Else
                        Dim strFile1 As String = "TEST"
                        Dim strCmd1 As String
                        strCmd1 = String.Format("alert('Please provide Advance amount.');window.opener.document.getElementById('" & Request.QueryString.Get("Retrive_id") & "').click();window.close();", strFile1)
                        ' strCmd1 = String.Format("window.close();", strFile1)
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "GEN1", strCmd1, True)
                        Exit Sub
                    End If
                   
                    'Response.Write("INSERT INTO SD_ORDER_CC_PAYMENT(ORDER_NO, LINE_NO, PAYMENT_TYPE,AMOUNT, CREATED_BY, CREATED_DATE)values('" & Order_no & "','" & LINE_NO & "','" & ptype & "','" & amt & "','" & Session("USER_ID") & "',SYSDATE)")
                    'Exit Sub
                End If
                ...
                'ds = Return_record_set("SELECT A.ORDER_NO,A.LINE_NO,round(A.AMOUNT,2) AMOUNT,b.Customer_no, b.EMAIL,b.NAME,NVL(b.BILLING_NAME,B.NAME) BILLING_NAME, NVL(b.BILLING_STREET,b.STREET) BILLING_STREET,b.STREET, NVL(b.BILLING_CITY,b.CITY) BILLING_CITY,b.CITY,NVL(b.BILLING_STATE_CODE,b.STATE_CODE) BILLING_STATE_CODE,b.STATE_CODE,NVL(b.BILLING_POSTAL_CODE,b.POSTAL_CODE) BILLING_POSTAL_CODE,b.POSTAL_CODE, NVL(b.BILLING_COUNTRY,b.COUNTRY) BILLING_COUNTRY ,b.COUNTRY,b.order_no,NVL(B.DIFFERENT_BILLING_ADDRESS,'N') DIFFERENT_BILLING_ADDRESS,B.CUSTOMER_PO_NO,to_char(B.INVOICE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DATE,to_char(B.INVOICE_DUE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DUE_DATE,b.CONTACT_NAME,D.ORDER_NO || '-' ||D.LINE_NO AS LINE_NO,D.ITEM_NO,D.DESCRIPTION,D.QTY,D.SALES_PRICE,D.PRICING_UOM,case when nvl(D.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(D.TAX_AMOUNT,0) tax_amount,D.LINE_TOTAL_AMOUNT,case when nvl(D.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=D.TAX_CODE),0) else 0 end as TAX_PER  FROM SD_ORDER_NO_CC_PAYMENT A, SD_ORDER_HEADER b,sd_customers C,SD_ORDER_DETAIL d WHERE A.ORDER_NO=B.ORDER_NO and b.order_no=d.order_no  and b.CUSTOMER_NO=C.account_no and  A.ORDER_NO='" & Order_no & "' ")


                'modified on 13 OCT 2020 by deepu
                'ds = Return_record_set("SELECT E.INVOICE_NO, A.ORDER_NO,A.LINE_NO,round(A.AMOUNT,2) AMOUNT,b.Customer_no, b.EMAIL,b.NAME,NVL(b.BILLING_NAME,B.NAME) BILLING_NAME, NVL(b.BILLING_STREET,b.STREET) BILLING_STREET,b.STREET, NVL(b.BILLING_CITY,b.CITY) BILLING_CITY,b.CITY,NVL(b.BILLING_STATE_CODE,b.STATE_CODE) BILLING_STATE_CODE,b.STATE_CODE,NVL(b.BILLING_POSTAL_CODE,b.POSTAL_CODE) BILLING_POSTAL_CODE,b.POSTAL_CODE, NVL(b.BILLING_COUNTRY,b.COUNTRY) BILLING_COUNTRY ,b.COUNTRY,b.order_no,NVL(B.DIFFERENT_BILLING_ADDRESS,'N') DIFFERENT_BILLING_ADDRESS,B.CUSTOMER_PO_NO,to_char(B.ORDER_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' ORDER_DATE,b.CONTACT_NAME,D.ORDER_NO || '-' ||D.LINE_NO AS LINE_NO,D.ITEM_NO,D.DESCRIPTION,D.ORDER_QTY,D.SALES_PRICE,D.PRICING_UOM,case when nvl(D.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(D.TAX_AMOUNT,0) tax_amount,D.LINE_TOTAL_AMOUNT,case when nvl(D.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=D.TAX_CODE),0) else 0 end as TAX_PER  FROM SD_ORDER_CC_PAYMENT A, SD_ORDER_HEADER b,sd_customers C,SD_ORDER_DETAIL d,SD_INVOICE_HEADER E WHERE A.ORDER_NO=B.ORDER_NO and b.order_no=d.order_no  and b.CUSTOMER_NO=C.account_no and  A.ORDER_NO='" & Order_no & "' AND A.ORDER_NO=E.ORDER_NO(+)")


'added by deepu on 28 OCT 2020
                'ds = Return_record_set("SELECT A.ORDER_NO,A.LINE_NO,round(A.AMOUNT,2) AMOUNT,null as CUSTOMER_PAYMENT_PROFILE_ID,A.PAYMENT_RESPONSE ,b.Customer_no,b.EMAIL,b.NAME,NVL(b.BILLING_NAME,B.NAME) BILLING_NAME,NVL(b.BILLING_STREET,b.STREET)BILLING_STREET,b.STREET, NVL(b.BILLING_CITY,b.CITY) BILLING_CITY,b.FREIGHT_BILLING_NAME, b.FREIGHT_BILLING_STREET,b.STREET, b.FREIGHT_BILLING_CITY,b.CITY,b.FREIGHT_BILLING_STATE_CODE,NVL(b.BILLING_STATE_CODE,b.STATE_CODE) BILLING_STATE_CODE,b.STATE_CODE,b.FREIGHT_BILLING_POSTAL_CODE,NVL(b.BILLING_POSTAL_CODE,b.POSTAL_CODE)BILLING_POSTAL_CODE,b.POSTAL_CODE, b.FREIGHT_BILLING_COUNTRY ,NVL(b.BILLING_COUNTRY,b.COUNTRY) BILLING_COUNTRY ,b.COUNTRY,B.CUSTOMER_PO_NO,to_char(B.ORDER_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' ORDER_DATE,b.CONTACT_NAME,D.ITEM_NO,D.DESCRIPTION,D.ORDER_QTY,D.SALES_PRICE,D.PRICING_UOM,case when nvl(D.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(D.TAX_AMOUNT,0) tax_amount,D.LINE_TOTAL_AMOUNT,case when nvl(D.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=D.TAX_CODE),0) else 0 end as TAX_PER FROM SD_ORDER_CC_PAYMENT A, SD_ORDER_HEADER b,sd_customers C,SD_ORDER_DETAIL d WHERE A.ORDER_NO=B.ORDER_NO AND b.order_no=d.order_no AND b.CUSTOMER_NO=C.account_no AND A.ORDER_NO='" & Order_no & "' AND A.LINE_NO=" & LINE_NO)
				
				
				ds = Return_record_set("SELECT A.ORDER_NO,A.LINE_NO,round(A.AMOUNT,2) AMOUNT,null as CUSTOMER_PAYMENT_PROFILE_ID,A.PAYMENT_RESPONSE ,sf_get_cc_customer_no(b.Customer_no) AS Customer_no,b.EMAIL,b.NAME,NVL(b.BILLING_NAME,B.NAME) BILLING_NAME,NVL(b.BILLING_STREET,b.STREET)BILLING_STREET,b.STREET, NVL(b.BILLING_CITY,b.CITY) BILLING_CITY,b.FREIGHT_BILLING_NAME, b.FREIGHT_BILLING_STREET,b.STREET, b.FREIGHT_BILLING_CITY,b.CITY,b.FREIGHT_BILLING_STATE_CODE,NVL(b.BILLING_STATE_CODE,b.STATE_CODE) BILLING_STATE_CODE,b.STATE_CODE,b.FREIGHT_BILLING_POSTAL_CODE,NVL(b.BILLING_POSTAL_CODE,b.POSTAL_CODE)BILLING_POSTAL_CODE,b.POSTAL_CODE, b.FREIGHT_BILLING_COUNTRY ,NVL(b.BILLING_COUNTRY,b.COUNTRY) BILLING_COUNTRY ,b.COUNTRY,B.CUSTOMER_PO_NO,to_char(B.ORDER_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' ORDER_DATE,b.CONTACT_NAME,D.ITEM_NO,D.DESCRIPTION,D.ORDER_QTY,D.SALES_PRICE,D.PRICING_UOM,case when nvl(D.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(D.TAX_AMOUNT,0) tax_amount,D.LINE_TOTAL_AMOUNT,case when nvl(D.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=D.TAX_CODE),0) else 0 end as TAX_PER FROM SD_ORDER_CC_PAYMENT A, SD_ORDER_HEADER b,sd_customers C,SD_ORDER_DETAIL d WHERE A.ORDER_NO=B.ORDER_NO AND b.order_no=d.order_no AND b.CUSTOMER_NO=C.account_no AND A.ORDER_NO='" & Order_no & "' AND A.LINE_NO=" & LINE_NO)
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    If (ds3.Tables("GetEbizWebFormURLResponse").Rows.Count > 0) Then
                        Update_query("UPDATE SD_ORDER_CC_PAYMENT SET API_TOKEN='" & replace_c(ds3.Tables("GetEbizWebFormURLResponse").Rows(0)("GetEbizWebFormURLResult")) & "' WHERE ORDER_NO='" & Order_no & "'")
                        If (ptype = "CC") Then
                            'If (ptype = "AUTH") Then
                ...
                Else
                    Dim strFile1 As String = "TEST"
                    Dim strCmd1 As String
                    strCmd1 = String.Format("alert('This Order is fully paid.');window.opener.document.getElementById('" & Request.QueryString.Get("Retrive_id") & "').click();window.close();", strFile1)
                    ' strCmd1 = String.Format("window.close();", strFile1)
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "GEN1", strCmd1, True)
                    Exit Sub
                End If
    ...
    Public Function SOAPManual(ByVal ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal url As String, ByVal action As String, ByVal TLSecurityID As String, ByVal TLClientURL As String, ByVal FromEmail As String, ByVal FromName As String) As String
        ...
        Ebiz_sales_payment_process_integration.InsertSoapEnvelopeIntoWebRequest(soapEnvelopeXml, webRequest)
    ...
    Private Shared Function CreateSoapEnvelope(ByRef ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal TL_USER_ID As String, ByVal TL_PASSWORD As String, ByVal TL_SECURITY As String, ByVal TL_CLIENT As String, ByVal TL_FROM_EMAIL As String, ByVal FromName As String) As XmlDocument
        ...
        For i = 0 To ds1.Rows.Count - 1
            ...
            item_details = item_details & "<ebiz:Qty>" & ds1.Rows(i)("ORDER_QTY").ToString() & "</ebiz:Qty>"
        ...
        "            <ebiz:EmailNotes>Order Payment</ebiz:EmailNotes>" &
        "            <ebiz:EmailSubject>Payment for Order No: " & ds1.Rows(0)("ORDER_NO").ToString() & "</ebiz:EmailSubject>" &
        ...
        "            <ebiz:TransId>" & ds1.Rows(0)("ORDER_NO").ToString() & "</ebiz:TransId>" &
        "            <ebiz:TransDetail>New Payment</ebiz:TransDetail>" &
        "            <ebiz:InvoiceNumber>" & ds1.Rows(0)("ORDER_NO").ToString() & "</ebiz:InvoiceNumber>" &
        ...
        "            <ebiz:Date>" & ds1.Rows(0)("ORDER_DATE").ToString() & "</ebiz:Date>" &
        ...
        "            </ebiz:BillingAddress>" &
        "            <ebiz:ApprovedURL>" & TL_CLIENT & "eBiz_cc_sales_process_pay_responce.aspx?PTYPE=" & ptype & "</ebiz:ApprovedURL>" &
        "            <ebiz:DeclinedURL>" & TL_CLIENT & "eBiz_cc_sales_process_pay_responce.aspx?PTYPE=" & ptype & "</ebiz:DeclinedURL>" &
        "            <ebiz:ErrorURL>" & TL_CLIENT & "eBiz_cc_sales_process_pay_responce.aspx?PTYPE=" & ptype & "</ebiz:ErrorURL>" &
        ...
        "            <ebiz:TransactionLookupKey>" & ds1.Rows(0)("ORDER_NO").ToString() & "</ebiz:TransactionLookupKey>" &
        item_details &
