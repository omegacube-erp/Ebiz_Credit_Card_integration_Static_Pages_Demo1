...
Imports System.Net
Imports System.Xml
Partial Class Ebiz_integration_Cash_Receipts
    Inherits System.Web.UI.Page
    Protected dbad As New OleDbDataAdapter
    Public conn As New Dbconn
    Public Record_no, line_no, resp, ptype As String
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Not Request.QueryString("RECORD_NO") Is Nothing Then
                ...
                Try
                    ...
                    End If
                Catch ex As Exception
                    CLIENT_URL = "https://demo1.omegacube.com"
                End Try
                ...
                Record_no = Request.QueryString.Get("RECORD_NO")
                ptype = Request.QueryString.Get("PAYMENT_TYPE")
				 Dim dsTransacId As New Data.DataSet
                dsTransacId = Return_record_set("select * from AR_CASH_RECEIPTS_CC_PAYMENT where RECORD_NO='" & Record_no & "' AND AUTHCODE IS NOT NULL")
                dsTransacId.Clear()
                ...
                If (dsTransacId.Tables(0).Rows.Count > 0) Then
                    ...
                    strCmd1 = String.Format("alert('This Cash Receipt is already Processed');window.opener.document.getElementById('" & Request.QueryString.Get("Retrive_id") & "').click();window.close();", strFile1)
                    ' strCmd1 = String.Format("window.close();", strFile1)
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "GEN1", strCmd1, True)
                    Exit Sub
                Else
                    Delete_query("DELETE FROM AR_CASH_RECEIPTS_CC_PAYMENT where RECORD_NO='" & Record_no & "'")
                    Insert_query("INSERT INTO AR_CASH_RECEIPTS_CC_PAYMENT (RECORD_NO,LINE_NO,AMOUNT,PAYMENT_TYPE,API_TOKEN,CREATED_BY,CREATED_DATE) select RECORD_NO,1,Amount,'" & ptype & "','','" & Session("USER_ID") & "',SYSDATE from AR_CASH_RECEIPTS_HEADER where  RECORD_NO='" & Record_no & "' and nvl(AMOUNT,0)>0")
                    'Response.Write("INSERT INTO AR_CASH_RECEIPTS_CC_PAYMENT (RECORD_NO,LINE_NO,INVOICE_NO,AMOUNT,PAYMENT_TYPE,API_TOKEN,CREATED_BY,CREATED_DATE) select RECORD_NO,1,INVOICE_NO,nvl(sum(PAYMENT_AMOUNT),0) + (select to_number(nvl(value,0)) from sys_tool_settings where key='DEFAULT_SHIPPING_CHARGE') + (nvl(sum(PAYMENT_AMOUNT),0) * (select to_number(nvl(value,0)) from sys_tool_settings where key='PERCENT_OF_EXTRA_CHARGES')/100)  Amount,'" & ptype & "','','" & Session("USER_ID") & "',SYSDATE,'AUTHONLY' from AR_CASH_RECEIPTS_DETAIL where  RECORD_NO='" & Record_no & "'  group by order_no")
                    'Exit Sub
                End If
                ...
                ds = Return_record_set("select A.RECORD_NO INVOICE_NO,A.RECORD_NO ORDER_NO, (SELECT P.AMOUNT FROM AR_CASH_RECEIPTS_HEADER P WHERE P.RECORD_NO=A.RECORD_NO) PAYMENT_AMOUNT,c.EMAIL,c.NAME,NVL(c.BILLING_NAME,c.NAME) BILLING_NAME, NVL(c.BILLING_STREET,c.STREET) BILLING_STREET,c.STREET, NVL(c.BILLING_CITY,c.CITY) BILLING_CITY,c.CITY,NVL(c.BILLING_STATE_CODE,c.STATE_CODE) BILLING_STATE_CODE,c.STATE_CODE,NVL(c.BILLING_POSTAL_CODE,c.POSTAL_CODE) BILLING_POSTAL_CODE,c.POSTAL_CODE, NVL(c.BILLING_COUNTRY,c.COUNTRY) BILLING_COUNTRY ,c.COUNTRY,c.order_no,NVL(c.DIFFERENT_BILLING_ADDRESS,'N') DIFFERENT_BILLING_ADDRESS,c.CUSTOMER_PO_NO,to_char(c.INVOICE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DATE,to_char(c.INVOICE_DUE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DUE_DATE,c.CONTACT_NAME,b.INVOICE_NO || '-' ||b.LINE_NO LINE_NO,b.ITEM_NO,b.DESCRIPTION,b.QTY,b.SALES_PRICE,b.PRICING_UOM,case when nvl(b.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(b.TAX_AMOUNT,0) tax_amount,b.LINE_TOTAL_AMOUNT,case when nvl(b.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=b.TAX_CODE),0) else 0 end as TAX_PER,sf_get_cc_customer_no(c.Customer_no) AS Customer_no from AR_CASH_RECEIPTS_DETAIL a,SD_INVOICE_DETAIL b,SD_INVOICE_HEADER c where a.RECORD_NO='" & Record_no & "' and a.INVOICE_NO=c.INVOICE_NO and c.INVOICE_NO=b.INVOICE_NO AND NVL(b.LINE_TOTAL_AMOUNT,0)>0")
                'Response.Write("select A.INVOICE_NO,c.ORDER_NO,round(A.PAYMENT_AMOUNT,2) PAYMENT_AMOUNT,c.EMAIL,c.NAME,NVL(c.BILLING_NAME,c.NAME) BILLING_NAME, NVL(c.BILLING_STREET,c.STREET) BILLING_STREET,c.STREET, NVL(c.BILLING_CITY,c.CITY) BILLING_CITY,c.CITY,NVL(c.BILLING_STATE_CODE,c.STATE_CODE) BILLING_STATE_CODE,c.STATE_CODE,NVL(c.BILLING_POSTAL_CODE,c.POSTAL_CODE) BILLING_POSTAL_CODE,c.POSTAL_CODE, NVL(c.BILLING_COUNTRY,c.COUNTRY) BILLING_COUNTRY ,c.COUNTRY,c.order_no,NVL(c.DIFFERENT_BILLING_ADDRESS,'N') DIFFERENT_BILLING_ADDRESS,c.CUSTOMER_PO_NO,to_char(c.INVOICE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DATE,to_char(c.INVOICE_DUE_DATE,'YYYY-MM-DD')||'T00:00:00-00:00' INVOICE_DUE_DATE,c.CONTACT_NAME,b.LINE_NO,b.ITEM_NO,b.INVOICE_NO || '-' ||b.LINE_NO AS LINE_NO,b.DESCRIPTION,b.QTY,b.SALES_PRICE,b.PRICING_UOM,case when nvl(b.TAX_AMOUNT,0)>0 then 'Y' ELSE 'N' END AS TAXABLE,nvl(b.TAX_AMOUNT,0) tax_amount,b.LINE_TOTAL_AMOUNT,case when nvl(b.TAX_AMOUNT,0)>0 then NVL((SELECT X.TAX_PERCENTAGE  FROM GE_STATE_CITY_TAX_PERCENTAGE X WHERE X.TAX_CODE=b.TAX_CODE),0) else 0 end as TAX_PER from AR_CASH_RECEIPTS_DETAIL a,SD_INVOICE_DETAIL b,SD_INVOICE_HEADER c where a.RECORD_NO='" & Record_no & "' and a.INVOICE_NO=c.INVOICE_NO and c.INVOICE_NO=b.INVOICE_NO")
                'Exit Sub
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    If (ds3.Tables("GetEbizWebFormURLResponse").Rows.Count > 0) Then
                        Update_query("UPDATE AR_CASH_RECEIPTS_CC_PAYMENT SET API_TOKEN='" & replace_c(ds3.Tables("GetEbizWebFormURLResponse").Rows(0)("GetEbizWebFormURLResult")) & "' WHERE RECORD_NO='" & Record_no & "'")
                        If (ptype = "CC") Then
    ...
    Public Function SOAPManual(ByVal ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal url As String, ByVal action As String, ByVal TLSecurityID As String, ByVal TLClientURL As String, ByVal FromEmail As String, ByVal FromName As String) As String
        ...
        Ebiz_integration_Cash_Receipts.InsertSoapEnvelopeIntoWebRequest(soapEnvelopeXml, webRequest)
    ...
    Private Shared Function CreateSoapEnvelope(ByRef ds1 As Data.DataTable, ByVal USER_ID As String, ByVal ptype As String, ByVal TL_USER_ID As String, ByVal TL_PASSWORD As String, ByVal TL_SECURITY As String, ByVal TL_CLIENT As String, ByVal TL_FROM_EMAIL As String, ByVal FromName As String) As XmlDocument
        ...
        "            <ebiz:TotalAmount>" & ds1.Rows(0)("PAYMENT_AMOUNT").ToString() & "</ebiz:TotalAmount>" &
        "            <ebiz:AmountDue>" & ds1.Rows(0)("PAYMENT_AMOUNT").ToString() & "</ebiz:AmountDue>" &
        ...
        "            <ebiz:ErrorURL>" & TL_CLIENT & "eBiz_Cash_Receipt_cc_responce.aspx?PTYPE=" & ptype & "</ebiz:ErrorURL>" &
